...
func (s *PublicBlockChainAPI) doCallByHash(ctx context.Context, args CallArgs, blockHash common.Hash, timeout time.Duration, globalGasCap *big.Int) ([]byte, uint64, bool, error) {
	state, header, err := s.b.StateAndHeaderByHash(ctx, blockHash)
	if state == nil || err != nil {
		return nil, 0, false, err
	}
	return s.call(ctx, args, state, header, timeout, globalGasCap)
}
...
func (s *PublicBlockChainAPI) call(ctx context.Context, args CallArgs, state *state.StateDB, header *types.Header, timeout time.Duration, globalGasCap *big.Int) ([]byte, uint64, bool, error) {
	defer func(start time.Time) { log.Debug("Executing EVM call finished", "runtime", time.Since(start)) }(time.Now())
...
func (s *PublicBlockChainAPI) CallByHash(ctx context.Context, args CallArgs, blockHash common.Hash) (hexutil.Bytes, error) {
	result, _, _, err := s.doCallByHash(ctx, args, blockHash, 5*time.Second, s.b.RPCGasCap())
	return (hexutil.Bytes)(result), err
}
